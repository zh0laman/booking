...
export default function BookingModal({ service, isOpen, onClose }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={step === 'form' ? 'Бронирование услуги' : 'Подтверждено'}>
      {step === 'form' ? (
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <button type="submit" className={styles.submitBtn}>
            {user ? 'Подтвердить бронирование' : 'Войти для бронирования'}
          </button>
        </form>
      ) : (
        <div className={styles.success}>
          ...
          <h3 className={styles.successTitle}>Бронирование подтверждено</h3>
          <p className={styles.successDesc}>
            Ваше бронирование <strong>{service.title}</strong> на {date} в {time} подтверждено.
          </p>
          <button onClick={handleClose} className={styles.submitBtn}>Готово</button>
        </div>
      )}
    </Modal>
  )
